...
def parse_arguments(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="transcribe",
        description="Transcribe a local audio or video file with Whisper",
...
Usage examples:
  ...
  %(prog)s aula.mp4 --detect-language
    ...
    parser.add_argument("source", help="Audio or video file to transcribe")
    parser.add_argument(
        "output",
        nargs="?",
...
def main(argv: Optional[list[str]] = None) -> int:
    try:
        ...
        service = TranscriptionService(
            config,
            on_progress=None if args.quiet else render_progress,
        )
