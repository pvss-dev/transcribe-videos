...
|---|---|---|
| Rate limit | nginx | 6 envios/min por IP, 12 conexões simultâneas |
| Cota por sessão | app | 10 transcrições/hora (`TRANSCRIPTOR_JOBS_PER_HOUR`) |
| Concorrência | app | **1 transcrição por vez** |
| Pasta fixa | app | o destino nunca vem do cliente |
| Retenção | app | apaga por idade e cota de disco |

Cada visitante enxerga só as próprias transcrições — o app separa por cookie de
...
Este serviço não busca nada no YouTube — só processa arquivos que o próprio
visitante envia —, então não esbarra nos bloqueios de IP de datacenter que
inviabilizam downloaders hospedados.
...
```bash
echo "TRANSCRIPTOR_NETWORK=o_nome_real" >> /root/projects/transcriptor/.env
```
...
```bash
# 1. Aponte transcription.pvss.dev.br (A/AAAA) para o IP da VPS
# 2. Comente o bloco `server { listen 443 ... }` do arquivo
docker exec nginx nginx -s reload

# 3. Emita o certificado
# --entrypoint certbot é obrigatório: o serviço certbot deste compose tem um
# entrypoint próprio (o loop de renovação), que engoliria estes argumentos.
docker compose run --rm --entrypoint certbot certbot \
  certonly --webroot -w /var/www/certbot -d transcription.pvss.dev.br
...
`github.com/pvss-dev/transcribe-videos` → **Settings** → **Environments** →
...
|---|---|
| `VPS_USER` | `root` |
| `VPS_APP_DIR` | `/root/projects/transcriptor` |
...
```bash
ssh-keygen -t ed25519 -C "github-deploy" -f ~/.ssh/transcriptor-deploy -N ""
ssh-copy-id -i ~/.ssh/transcriptor-deploy.pub root@SEU_IP
cat ~/.ssh/transcriptor-deploy        # <- este conteúdo vai no VPS_SSH_KEY
```
...
| Volume | Conteúdo |
|---|---|
| `transcriptor-out` | transcrições (.txt e .srt) |
| `transcriptor-cache` | pesos do Whisper (~500 MB para o `small`) |
...
```bash
TRANSCRIPTOR_RETENTION_DAYS=15
TRANSCRIPTOR_RETENTION_MAX_GB=10
TRANSCRIPTOR_MEMORY_LIMIT=4g
```
...
```bash
docker compose logs -f transcribe-videos     # acompanhar
docker compose ps                        # estado e saúde
docker compose down                      # parar (volumes ficam)

# Voltar a uma versão anterior sem esperar build:
TRANSCRIPTOR_IMAGE=ghcr.io/pvss-dev/transcribe-videos:<sha> docker compose up -d
```
...
- **Uma transcrição por vez.** As demais esperam até 30 minutos por uma vaga e
  então falham com "servidor ocupado". Aumente `TRANSCRIPTOR_MAX_CONCURRENT` se
  a VPS tiver CPU sobrando.
